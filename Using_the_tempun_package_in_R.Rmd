...
_Inscriptions as data: digital epigraphy in macro-historical perspective_ in R. This is done by using the reticulate package. I have recreated figures 1-3 as a starting point. Another document on the reticulate package can be found in my github at: INSERT LINK 

## Setup 
```{r chunk 1, setup in R}

#install.packages(c("tidyverse", "reticulate", "jsonlite"))
...
#use_condaenv("RStudio") # Use this to use a conda environment if you have one
use_python("/usr/local/bin/python") # use this if you have local python installed

# load Python packages - via terminal window, TBA
...
When importing the r-object EDH into python I add r. before the name of the object. Then reticulate handles the rest.
```{python}
...
Here I am saving the python object EDH_dated as an R-object called EDH_dated by using  py$ from the reticulate package. 
```{r}
# saving as r-object
