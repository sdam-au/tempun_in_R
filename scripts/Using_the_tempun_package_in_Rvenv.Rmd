---
title: "Using the tempun-package in R with Python venv"
author: "Adela"
date: "9 Nov 2022"
...
#use_virtualenv() # LOOK AT THE SETUP BELOW!
#use_condaenv("pip") # Use this to use a conda environment if you have one
#use_python("C:/Users/adela/AppData/Local/Programs/Python/Python39/") # use this if you have local python installed
```
Create Python virtual env in R and use that
```{r}
# Set up virtual environment for python packages
virtualenv_create(envname = "venv", python = NULL ,packages = c("geopandas", "requests","seaborn","matplotlib", "scipy", "tempun", "numpy", "pandas"))
...
# If messy, delete venv and restart
#virtualenv_remove("venv")

# If needing, you can install additional packages into the venv (BUT JSON IS NOT NEEDED)
# virtualenv_install(envname = "venv", packages = c("json"), ignore_installed = TRUE)
...
fig
```
Adela : I do not get why the next chunk uses the 'simulation' language. Does it derive the values from the simulations above? I don't see the simulations happening here and they don't seem necessary for the table summary
```{python}
...
In the following chunk we are loading a shapefile, that represents the Roman Empire at its largest extent in AD 117. This is used in the third figure. 
```{python}
